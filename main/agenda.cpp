...
void inicializa_agenda(void) {
    Memoria::escreve(0, {0, 0}, (uint16_t*)sizeof(uint16_t));   //Zera quantidade de registros no índice da memória
    return;
}
...
void pesquisa_registro_nome(char nome_registro[20]){
    ...
    Memoria::le(0, &num, (uint16_t*)2*sizeof(uint8_t));  //Lê a quantidade de registros
    index = (uint16_t*)num[0] << 8;
    index = index + (uint16_t*)num[1];

    if(index == 0){   //Checa quantidade de registros
        printf("Não há registros para pesquisar!");
    }
    else{
        for(uint16_t i = 0; i < index; i++){
            Memoria::le(2+i*sizeof(tipo_registro), buffer_memoria, (uint16_t*)sizeof(tipo_registro));   //Lê o endereço na memória de cada registro 
...
void remove_registro(char& telefone_registro){
    ...
    if((uint16_t*)num == 0){   //Checa quantidade de registros
        ...
        for(uint16_t i = 0; i < (uint16_t*)num; i++){
            Memoria::le(2+i*sizeof(tipo_registro), buffer_memoria, (uint16_t*)sizeof(tipo_registro));   //Lê o endereço na memória de cada registro 
            ...
            if(buffer.telefone == telefone_registro){  //Verifica se o nome fornecido é igual a algum nome registrado
                ...
                Memoria::le(2+index*sizeof(tipo_registro), buffer_memoria, (uint16_t*)sizeof(tipo_registro));  //Guarda o último registro da agenda

                Memoria::escreve(2+pos, buffer_memoria, (uint16_t*)sizeof(tipo_registro));  //Escreve o último registro da agenda sobre o registro removido

                (uint16_t*)num-=1;
                Memoria::escreve(0, &num, (uint16_t*)sizeof(uint16_t)); //Atualiza a o índice da agenda
